    ...
    public static loadEndpointHtml(endpointUrl: string, queryParams?: any): Promise<string> {
        ...
        var promise = new Promise<string>((resolve, reject) => {
            ...
            request(opts, (__, ___, data) => {
                if (data == undefined) {
                    reject(new DestinyLib.Errors.Exception('No result returned from call to load from endpoint "' + endpointUrl + '". Check to make sure that you are connected to the internet.'));
                    return;
                }
